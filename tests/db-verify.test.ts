...
describe("db:verify read-only checks", () => {
  ...
  it("expects platform table count 72 after migration 0015", () => {
    assert.equal(EXPECTED_PLATFORM_TABLE_COUNT, 72);
  ...
  it("passes when platform table count is 72", async () => {
    const result = await verifyPlatformDatabase(
      mockQuery({ platformTableCount: 72 }),
    );
    assert.equal(result.ok, true);
    assert.equal(
